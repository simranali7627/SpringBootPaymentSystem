...
import com.example.accolite.repository.*;
import lombok.Data;
...
import static com.example.accolite.entity.TransactionCategory.*;
...
public class TransactionController {
    ...
    public ResponseEntity<String> makePayment(@RequestBody PaymentRequestOnline paymentRequest) {
        ...
        if (!isWithinRadius(paymentRequest.getLatitude(), paymentRequest.getLongitude(), vendor.getLatitude(), vendor.getLongitude(), 20)) {
            ...
            transaction.setTransactionCategory(FLAGGED);
            ...
            userWallet.setwalletBalance(userWallet.getwalletBalance() - paymentRequest.getAmount());
            ...
            transaction.setVendorId(paymentRequest.getVendorId());
            transaction.setAmount(paymentRequest.getAmount());
            transaction.setTransactionCategory(SUCCESSFUL);
            transaction.setPaymentMode(PaymentMode.ONLINE);
            transaction.setTransactionDate(new Date());
            ...
            vendorWallet.setwalletBalance(vendorWallet.getwalletBalance() + paymentRequest.getAmount());
    ...
    public ResponseEntity<String> makePaymentOffline(@RequestBody PaymentRequestOffline paymentRequestOffline) {
        ...
        if (!isWithinRadius(paymentRequestOffline.getLatitude(), paymentRequestOffline.getLongitude(), vendor.getLatitude(), vendor.getLongitude(), 20)) {
            ...
            // Update user's wallet balance
            Wallet userWallet = user.getWallet();
            userWallet.setofflineWalletBalance(userWallet.getofflineWalletBalance() - paymentRequestOffline.getAmount());
            walletRepository.save(userWallet);

            // Update vendor's wallet balance
//            Wallet vendorWallet = vendor.getPersonalWallet();
//            vendorWallet.setBalance(vendorWallet.getwalletBalance() + paymentRequestOffline.getAmount());
//            walletRepository.save(vendorWallet);
            ...
            // Update user's wallet balance
            Wallet userWallet = user.getWallet();
            userWallet.setwalletBalance(userWallet.getofflineWalletBalance() - paymentRequestOffline.getAmount());
            walletRepository.save(userWallet);

            // Update vendor's wallet balance
            Wallet vendorWallet = vendor.getStoreWallet();
            vendorWallet.setwalletBalance(vendorWallet.getwalletBalance() + paymentRequestOffline.getAmount());
            walletRepository.save(vendorWallet);
    ...
    public ResponseEntity<List<Transaction>> getFlaggedTransactions() {
        ...
        List<Transaction> flaggedTransactions = transactionRepository.findByStatus(TransactionCategory.FLAGGED).get();
    ...
    public ResponseEntity<String> reviewTransaction(
        ...
        Transaction transaction = (Transaction) transactionRepository.findById(transactionId)
                .orElseThrow(() -> new RuntimeException("Transaction not found"));

        if (transaction.getTransactionCategory() == FLAGGED) {
            if (approval) {
                // Approve transaction
    ...
    private void transferAmountToVendor(Transaction transaction) {
        ...
        Wallet vendorWallet = vendor.getStoreWallet();
        vendorWallet.setwalletBalance(vendorWallet.getwalletBalance() + transaction.getAmount());
    ...
    private void returnAmountToUser(Transaction transaction) {
        ...
        Wallet userWallet = user.getWallet();
        userWallet.setwalletBalance(userWallet.getwalletBalance() + transaction.getAmount());

        // Save the updated user's wallet
        walletRepository.save(userWallet);

        // Update the transaction status to FAILED
        transaction.setTransactionCategory(FAILED);
        transactionRepository.save(transaction);
    }
